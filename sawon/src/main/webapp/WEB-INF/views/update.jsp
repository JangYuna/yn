...
var pattern = /.{6}$/;	
var jumin = $("#insa_reg_no2").val();	
var jumin2 = jumin.replace(pattern, "******");

$("#insa_reg_no").val(jumin2);	
...
$("#cmp_reg_image_bt").click(function() {
	...
	var insa_cmp_reg_image = $("#insa_cmp_reg_image").val();
	
	if(insa_cmp_reg_image==""){
		alert("등록된 파일이 없습니다");
	}else{
...
$("#insa_cmp_reg_image2").change(function(){
	...
	var inputFile = $("input[name='insa_cmp_reg_image2']");
	var files = inputFile[0].files;
	
	var insaSabun = $("#insa_sabun").val();
	
	console.log(files);
	
	formData.append("insaSabun", insaSabun);
...
$("#carrier_bt").click(function() {
	...
	var insa_carrier = $("#insa_carrier").val();
...
$("#insa_carrier2").change(function(){
	...
	var formData = new FormData();
	var inputFile = $("input[name='insa_carrier2']");
	var files = inputFile[0].files;
...
function checkDate2(){
	var ckjoinDate = $("#insa_mil_startdate").val();
	var jArr = ckjoinDate.split('/');
	console.log("ckjoinDate:"+ckjoinDate);
	console.log("jArr:"+jArr);
	
	var ckretireDate = $("#insa_mil_enddate").val();
	var rArr = ckretireDate.split('/');
	console.log("ckretireDate:"+ckretireDate);
	console.log("rArr:"+rArr);
...
function sawonddelete(){
	
	var insasabun = $("#insa_sabun").val();	
	var currentPage = $("#currentPage").val();
	
	console.log("insa_sabun:"+insasabun);
	console.log("currentPage:"+currentPage);
	
	if(!confirm('선택한 사원을 삭제하시겠습니까?')){
		return false;
	}else{
		return location.href='memberdelete.do?insa_sabun='+insasabun+'&currentPage='+currentPage;	
	}
...
function down1(){
	...
	if(insa_cmp_reg_image == ""){
		...
		return location.href='cmpregimagedw.do?insa_cmp_reg_image='+insa_cmp_reg_image;	
	}
...
function down2(){
	...
	if(insa_carrier == ""){
		...
		return location.href='carrierdw.do?insa_carrier='+insa_carrier;	
	}
...
<nav class="navbar navbar-expand-sm bg-light navbar-light">
  <div class="container-fluid">
	...
	<br>
	<form id="dataForm" enctype="multipart/form-data" method="post" action="memberupdate.do" onsubmit="return signIn();">
	<div class="col-xl text-right">
		<button id="uploadBtn" name="uploadBtn" class="btn btn-primary mr-1">수정</button>
		<button type="button" class="btn btn-primary mr-1" onclick="return sawonddelete();">삭제</button>
		<button type="button" id="reset" class="btn btn-primary mr-1" onclick="goBack()">이전</button>
	</div>
	<br>
    <table>
    	<tr>
    		...
    		<c:choose>
    		<c:when test="${empty insa.insa_profile }">
    		<img src="resources/images/base_profile.png" id="profile_img_2" name ="profile_img_2" style="width : 180px; height : 180px;">
    		</c:when>
    		<c:otherwise>
			<img src="resources/images/imagefile/${insa.insa_profile }" id="profile_img_2" name ="profile_img_2" style="width : 180px; height : 180px;">/    		
    		</c:otherwise>
    		</c:choose>
    		<br>
    		...
    		<input type="hidden" id="insa_profile" name="insa_profile" value="${insa.insa_profile }">
    		</td>
    		<td>
    		<div class="textbox">
    		<label>*사번&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label>
            <input type="text" id="insa_sabun" name="insa_sabun" value="${insa.insa_sabun }" readonly>
            </div>
    		</td>
    		<td>
    		<div class="textbox">
    		<label>*한글성명&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label>
            <input type="text" id="insa_name" name="insa_name" value="${insa.insa_name }">
            </div>
    		</td>
    		<td>
    		<div class="textbox">
    		<label>영문성명&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label>
            <input type="text" id="insa_eng_name" name="insa_eng_name" value="${insa.insa_eng_name }">
            </div>
    		</td>
    	</tr>
    	<tr>
    		...
    		<div class="textbox">
    		<label>*아이디&nbsp;&nbsp;&nbsp;&nbsp;</label>
            <input type="text" id="insa_id" name="insa_id" value="${insa.insa_id }" readonly>
            </div>
    		</td>
    		<td>
    		<div class="textbox">
    		<label>*패스워드&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label>
            <input type="password" id="insa_pwd" name="insa_pwd" value="${insa.insa_pwd }">
            </div>
    		</td>
    		<td>
    		<div class="textbox">
    		<label>*패스워드확인</label>
             <input type="password" id="insa_pwd2" name="insa_pwd2" value="${insa.insa_pwd }" onchange="pwdchecking()">
            </div>
    		</td>
    	</tr>
    	<tr>
    		...
    		<div class="textbox">
    		<label>전화번호&nbsp;&nbsp;</label>
            <input type="text" id="insa_phone" name="insa_phone" value="${insa.insa_phone }" onKeyup="inputPhoneNumber(this);">
            </div>
    		</td>
    		<td>
    		<div class="textbox">
    		<label>*핸드폰번호&nbsp;&nbsp;</label>
            <input type="text" id="insa_hp" name="insa_hp" value="${insa.insa_hp }" onKeyup="inputPhoneNumber(this);">
            </div>
    		</td>
    		<td>
    		<div class="textbox">
            ...
            <input type="hidden" id="insa_reg_no2" name="insa_reg_no2" value="${insa.insa_reg_no }">
            </div>
    		</td>
    	</tr>
    	<tr>
    		...
    		<div class="textbox">
    		<label>연령&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label>
            <input type="text" id="insa_years" name="insa_years" value="${insa.insa_years }">
            </div>
    		</td>
    		<td>
    		<div class="textbox_email">    		
    		<label>*이메일</label> 		
            <input type="text" id="insa_email" name="insa_email" value="${email1}">
            <input type="text" id="insa_email2" name="insa_email2" value="${email2 }">
            <c:choose>                       
    		...
    		<c:forEach var="insacom" items="${list }">
            		<c:if test="${insacom.note ==  'insa_join_gbn_code'}">
            			<option value="${insacom.name }" <c:if test="${ insacom.name == insa.insa_join_gbn_code }">selected</c:if>>${insacom.name }</option>
            		</c:if>
            </c:forEach>
            </select>
            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
    		...
    		<c:forEach var="insacom" items="${list }">
            		<c:if test="${insacom.note ==  'insa_sex'}">
            			<option value="${insacom.name }" <c:if test="${ insacom.name == insa.insa_sex }">selected</c:if>>${insacom.name }</option>
            		</c:if>
            </c:forEach>
            </select>
            </div>
    	...
    	<tr>
    		...
    		<label>주소&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label>
            <input type="text" id="insa_zip" name="insa_zip" value="${insa.insa_zip }" placeholder="우편번호">
            &nbsp;&nbsp;&nbsp;
            ...
            <input type="text" id="insa_addr1" name="insa_addr1" value="${insa.insa_addr1 }" placeholder="주소">
            </div>
    		</td>
    		<td>
    		<div class="textbox_addr">
            <input type="text" id="insa_addr2" name="insa_addr2" value="${insa.insa_addr2 }" placeholder="세부주소">
            </div>
    		</td>
    	</tr>
    	<tr>
            ...
            <select id="insa_pos_gbn_code" name="insa_pos_gbn_code">
            	...
            	<c:forEach var="insacom" items="${list}">
					<c:if test="${insacom.note == 'insa_pos_gbn_code'}">
						<option value="${insacom.name}" <c:if test="${ insacom.name == insa.insa_pos_gbn_code }">selected</c:if>>${insacom.name}</option>
					</c:if>
				</c:forEach>
            </select>
            </div>
            ...
            <select id="insa_dept_code" name="insa_dept_code">
            	...
            	<c:forEach var="insacom" items="${list}">
					<c:if test="${insacom.note == 'insa_dept_code'}">
						<option value="${insacom.name}" <c:if test="${ insacom.name == insa.insa_dept_code }">selected</c:if>>${insacom.name}</option>
					</c:if>
				</c:forEach>
            </select>
            </div>
            ...
            <input type="text" id="insa_salary" name="insa_salary" value="${insa.insa_salary }" style='width:75%; border:0px; text-align:right;'>
            (만원)</div>
            </div>
    		</td>
    	</tr>
            ...
            <select id="insa_join_type" name="insa_join_type">
            	...
            	<c:forEach var="insacom" items="${list}">
					<c:if test="${insacom.note == 'insa_join_type'}">
						<option value="${insacom.name}" <c:if test="${ insacom.name == insa.insa_join_type }">selected</c:if>>${insacom.name}</option>
					</c:if>
				</c:forEach>
            </select>
            </div>
            ...
            <select id="insa_gart_level" name="insa_gart_level">
            	...
            	<c:forEach var="insacom" items="${list}">
					<c:if test="${insacom.note == 'insa_gart_level'}">
						<option value="${insacom.name}" <c:if test="${ insacom.name == insa.insa_gart_level }">selected</c:if>>${insacom.name}</option>
					</c:if>
				</c:forEach>
            </select>
            </div>
            ...
            <select id="insa_put_yn" name="insa_put_yn">
            	...
            	<c:forEach var="insacom" items="${list}">
					<c:if test="${insacom.note == 'insa_put_yn'}">
						<option value="${insacom.name}" <c:if test="${ insacom.name == insa.insa_put_yn }">selected</c:if>>${insacom.name}</option>
					</c:if>
				</c:forEach>
            </select>
            </div>
            ...
            <select id="insa_mil_yn" name="insa_mil_yn">
            	...
            	<c:forEach var="insacom" items="${list}">
					<c:if test="${insacom.note == 'insa_mil_yn'}">
						<option value="${insacom.name}" <c:if test="${ insacom.name == insa.insa_mil_yn }">selected</c:if>>${insacom.name}</option>
					</c:if>
				</c:forEach>
            </select>
            </div>
            ...
            <option value="">선택</option>
            	<c:forEach var="insacom" items="${list}">
					<c:if test="${insacom.note == 'insa_mil_type'}">
						<option value="${insacom.name}" <c:if test="${ insacom.name == insa.insa_mil_type }">selected</c:if>>${insacom.name}</option>
					</c:if>
				</c:forEach>
            </select>
            <input type="hidden" id="insa_mil_type" name="insa_mil_type" value="${insa.insa_mil_type }">
            </div>
    		</td>
    		<td>
    		<div class="textbox">
            ...
            <select id="insa_mil_level2" name="insa_mil_level2">
            	...
            	<c:forEach var="insacom" items="${list}">
					<c:if test="${insacom.note == 'insa_mil_level'}">
						<option value="${insacom.name}" <c:if test="${ insacom.name == insa.insa_mil_level }">selected</c:if>>${insacom.name}</option>
					</c:if>
				</c:forEach>
            </select>
            <input type="hidden" id="insa_mil_level" name="insa_mil_level" value="${insa.insa_mil_level }">
            </div>
    		</td>
    		<td>
    		<div class="textbox_date">
    		<label>입영일자&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label>
            <input type="text" id="insa_mil_startdate2" name="insa_mil_startdate2" value="${insa.insa_mil_startdate }" onchange="checkDate2();">
            <input type="hidden" id="insa_mil_startdate" name="insa_mil_startdate" value="${insa.insa_mil_startdate }">
            </div>
    		</td>
    		<td>
    		<div class="textbox_date">
    		<label>전역일자&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label>
            <input type="text" id="insa_mil_enddate2" name="insa_mil_enddate2" value="${insa.insa_mil_enddate }" onchange="checkDate2();">
            <input type="hidden" id="insa_mil_enddate" name="insa_mil_enddate" value="${insa.insa_mil_enddate }">
            </div>
    		</td>
    	</tr>
    	<tr>
            ...
            <select id="insa_kosa_reg_yn" name="insa_kosa_reg_yn">
            	...
            	<c:forEach var="insacom" items="${list}">
					<c:if test="${insacom.note == 'insa_kosa_reg_yn'}">
						<option value="${insacom.name}" <c:if test="${ insacom.name == insa.insa_kosa_reg_yn }">selected</c:if>>${insacom.name}</option>
					</c:if>
				</c:forEach>
            </select>
            </div>
            ...
            <select id="insa_kosa_class_code2" name="insa_kosa_class_code2">
            	...
            	<c:forEach var="insacom" items="${list}">
					<c:if test="${insacom.note == 'insa_kosa_class_code'}">
						<option value="${insacom.name}" <c:if test="${ insacom.name == insa.insa_kosa_class_code }">selected</c:if>>${insacom.name}</option>
					</c:if>
				</c:forEach>
            </select>
            <input type="hidden" id="insa_kosa_class_code" name="insa_kosa_class_code" value="${insa.insa_kosa_class_code }">
            </div>
    		</td>
    		<td>
    		<div class="textbox_date">
    		<label>*입사일자&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label>
            <input type="text" id="insa_join_day" name="insa_join_day" value="${insa.insa_join_day }" onchange="checkDate();">
            </div>
    		</td>
    		<td>
    		<div class="textbox_date">
    		<label>퇴사일자&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label>
            <input type="text" id="insa_retire_day" name="insa_retire_day" value="${insa.insa_retire_day }" onchange="checkDate();">
            </div>
    		</td>
    	</tr>
    	<tr>
    		...
    		<label>사업자번호</label>
            <input type="text" id="insa_cmp_reg_no" name="insa_cmp_reg_no" value="${insa.insa_cmp_reg_no }">
            </div>
    		</td>
    		<td>
    		<div class="textbox">
    		<label>업체명&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label>
            <input type="text" id="insa_crm_name" name="insa_crm_name" value="${insa.insa_crm_name }">
            </div>
    		</td>
    		<td>
    		<div class="textbox">
    		<label>사업자등록증</label>
            <input type="text" id="insa_cmp_reg_image" name="insa_cmp_reg_image" value="${insa.insa_cmp_reg_image }" readonly>
            </div>
    		</td>
    		<td>
    		<div class="textbox">
...
<%--     	<c:url var="cmpdown" value="cmpregimagedw.do">
			<c:param name="cmp_reg_image_pr">${insa_cmp_reg_image}</c:param>
			</c:url> --%>
			<button type="button" class="btn btn-outline-primary" id="cmp_image_dw" onclick="return down1();">다운로드</button>
    	...
    	<tr>
    		...
    		<div class="textbox">
    		<label>자기소개&nbsp;&nbsp;&nbsp;&nbsp;</label>
            <textarea cols="80" rows="1" id="insa_self_intro" name="insa_self_intro" placeholder="100자 내외로 적으시오">${insa.insa_self_intro }</textarea>
            </div>
    		</td>
    		<td>
    		<div class="textbox">
    		<label>이력서&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label>
            <input type="text" id="insa_carrier" name="insa_carrier" value="${insa.insa_carrier }" readonly>
            </div>
    		</td>
    		<td>
    		<div class="textbox">
...
<%--     	<c:url var="carrierdown" value="carrierdw.do">
			<c:param name="carrier_pr">${insa.insa_carrier}</c:param>
			</c:url> --%>
			<button type="button" class="btn btn-outline-primary" id="carrier_dw" onclick="return down2();">다운로드</button>
  ...
  <div class="modal fade" id="cmp_reg_image_modal">
    <div class="modal-dialog modal-sm">
      <div class="modal-content">  
        <!-- Modal body -->
          <img id="cmp_reg_image" src="resources/images/imagefile/${insa.insa_cmp_reg_image }"/>
	   </div>
    </div>
  </div>
    <!-- The Modal -->
  <div class="modal fade" id="carrier_modal">
    <div class="modal-dialog modal-sm">
      <div class="modal-content">  
        <!-- Modal body -->
          <img id="carrier_image" src="resources/images/imagefile/${insa.insa_carrier }" />        
      </div>
    </div>
  </div>
</body>
